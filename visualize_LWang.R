...
bikecounts %>% 
  group_by(brname, month) %>% 
  summarize(monthly_total=sum(total)) %>% 
  ggplot() +
  geom_line(aes(x=month, y=monthly_total, group=brname, color=name))

ggplot(bikecounts) +
  geom_col(aes(x=date, y=total, color=brname))

ggplot(bikecounts) +
  geom_boxplot(aes(x=month, y=total))

bikecounts %>% 
  group_by(brname, week) %>% 
  summarize(weekly_total=sum(total)) %>% 
  ggplot() +
  geom_line(aes(x=week, y=weekly_total, color=brname))
...
bikecounts_weather %>% 
  ggplot() +
  geom_smooth(aes(x=TMIN, y=total, group=brname)) +
  geom_smooth(aes(x=TMAX, y=total, group=brname)) +
  facet_wrap(~name)
...
library(ggfortify)
hawthorne_bc <- bikecounts %>% filter(brname=="Hawthorne") %>% pull(total)
